...
export default {
    ...
    computed: {
        filtered: function (){
            let sorted = this.recepients
            if (this.$root.filter === 'All'){
                return sorted
            } else if (this.$root.filter === 'Opened'){
                return sorted.filter(e => e.opened === 1)
            } else if (this.$root.filter === 'Opened, unreplied'){
                return sorted.filter(e => e.opened === 1 && e.replied === 0)
            } else if (this.$root.filter === 'Replied'){
                return sorted.filter(e => e.replied === 1)
            } else if (this.$root.filter === 'Unopened'){
                return sorted.filter(e => e.opened === 0)
            }
        }
    },
