...
class SendEmailsGoogle extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'email:google';
     ...
     * @var string
     */
    protected $description = 'Send email on google';
    ...
    {
        $users = User::where('type', 'Gmail')->with('token')->get();
        foreach ($users as $user){
            $sequences = Sequences::where('user_id', $user->id)->with('recepients')->with('stages')->get();
                ...
                foreach ($sequences as $sequence){
                    ...
                    if ($stageCount < 2){
                        ...
                        foreach ($checkList as $recepient){
                            ...
                            echo strtotime($recepient->updated_at) . "\r\n";
                        }
                    } else {
                        foreach ($checkList as $recepient){
                            $endTime = date('d M Y H:I:s', strtotime('+' . $sequence->stages[$recepient->stage + 1]->time . ' ' . $sequence->stages[$recepient->stage + 1]->type_time));
                            if (strtotime($recepient->updated_at) <= strtotime($endTime)){
                                array_push($sendList, $recepient);
                            }
                        }
